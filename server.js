...
const MAX_PAYEE_PAGES = Number(process.env.ROADSYNC_MAX_PAYEE_PAGES || 20);
...
function compareTransactionToDotPayee(tx, payee) {
  ...
  const dotPayeeId = String(payee?.id || "");
...
async function fetchPayeesPage(page) {
  const endpoint = `/payees?page=${page}`;
  const raw = await rspayGet(endpoint);
  const rows = toArray(raw);

  console.log(`RSPay payees page ${page} returned:`, rows.length);
  return rows;
}
...
async function findPayeeByDot(dot) {
  ...
  for (let page = 1; page <= MAX_PAYEE_PAGES; page += 1) {
    const items = await fetchPayeesPage(page);

    const exactDotMatch = items.find(
      item => normalizeIdLike(item?.dot_number) === targetDot
    );

    if (exactDotMatch) {
      console.log("Found payee by DOT on page:", page);
      return exactDotMatch;
    }
...
app.get("/api/test-payees", async (req, res) => {
  try {
    ...
    const payee = await findPayeeByDot(dot);

    res.json({
      ok: true,
      dot,
      found: Boolean(payee),
      payee: payee ? summarizePayee(payee) : null
    });
  } catch (err) {
  ...
  try {
    ...
    if (firstReferenceMatch && !payeeByDot) {
      return res.json({
        ...
        debug: {
          message: "A payment matched the reference, but the DOT number was not found in RSPay payees."
        }
      });
    }
